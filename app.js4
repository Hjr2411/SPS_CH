// app.js — SPS_Ch 1.1 (substitua seu arquivo por este)
...
// ======================= STATE =======================
const state = {
  charts: {equip:null, cenario:null, analista:null},
...
function applyAuthUI(){
  ...
  if(!s){
    ...
    navUser.hidden = true; navAdmin.hidden = true; navLogin.hidden = false;
    showView('loginView');
  } else {
    userBadge.textContent = `${s.nome}${s.admin?' (admin)':''}`;
    btnLogout.hidden = false;
    navLogin.hidden = true; navUser.hidden = false; navAdmin.hidden = !s.admin;
...
Array.from(document.querySelectorAll('.nav-btn')).forEach(btn=>{
  btn.addEventListener('click', ()=>{
    const v = btn.dataset.view; if(v) showView(v);
  });
});
...
function upsertBarChart(inst, canvasId, obj){
  ...
  return new Chart(ctx,{ type:'bar', data:{ labels, datasets:[{ label:'Qtd', data }] }, options:{ responsive:true, plugins:{ legend:{display:false}, tooltip:{enabled:true} }, scales:{ y:{ beginAtZero:true } } } });
}
function upsertPieChart(inst, canvasId, obj){
  const ctx = document.getElementById(canvasId);
  const labels = Object.keys(obj); const data = Object.values(obj);
  if(inst){ inst.data.labels=labels; inst.data.datasets[0].data=data; inst.update(); return inst; }
  return new Chart(ctx,{ type:'doughnut', data:{ labels, datasets:[{ data }] }, options:{ responsive:true, plugins:{ legend:{position:'bottom'} } } });
}
